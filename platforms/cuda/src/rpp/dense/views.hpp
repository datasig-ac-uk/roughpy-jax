...
public:
    RPP_HOST_DEVICE
    explicit constexpr DenseVectorView(It_ data, Basis const &basis)
        : data_(data), min_degree_(0), max_degree_(basis.depth), basis_(basis) {
    }

    RPP_HOST_DEVICE
    explicit constexpr DenseVectorView(It_ data, Basis const &basis, Degree min_deg, Degree max_degree)

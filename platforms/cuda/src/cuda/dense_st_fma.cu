...
ffi::Error cuda_dense_st_fma_impl(cudaStream_t stream,
    ...
    return select_type_and_go<DenseSTFmaFunctor>(
        out->element_type(), out, a, b, c, static_args, stream);
}

ffi::Error cuda_dense_st_mul_impl(cudaStream_t stream,
                                  ffi::Result<ffi::AnyBuffer> out,
                                  ffi::AnyBuffer lhs,
                                  ffi::AnyBuffer rhs,
                                  int32_t width,

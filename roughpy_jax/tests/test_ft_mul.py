...
def test_ft_mul_explicit_out_basis(out_depth):
    ...
    t_lhs = rpj.FreeTensor(jnp.ones(input_basis.size()), input_basis)
...
def test_ft_mul_rejects_incompatible_out_basis():
    basis = rpj.TensorBasis(2, 2)
    argument = rpj.FreeTensor(jnp.ones(basis.size()), basis)

    with pytest.raises(ValueError, match="Incompatible width"):
        rpj.ft_mul(argument, argument, out_basis=rpj.TensorBasis(3, 2))
